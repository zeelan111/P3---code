...
# Step 1: Compute Eigenvector Centrality and Normalize using PyTorch
print("Computing eigenvector centralities...")
eigenvector_centralities = nx.eigenvector_centrality(nx_graph)  # Compute eigenvector centrality
centrality_values = torch.tensor(list(eigenvector_centralities.values()), dtype=torch.float)  # Convert to tensor

# Normalize the eigenvector centralities to range [0, 1]
min_centrality = torch.min(centrality_values)
max_centrality = torch.max(centrality_values)
normalized_centralities = (centrality_values - min_centrality) / (max_centrality - min_centrality)

# Map string node IDs to consecutive integer indices
node_mapping = {node_id: idx for idx, node_id in enumerate(eigenvector_centralities.keys())}

# Create a feature matrix where each node's feature is its normalized eigenvector centrality
centrality_features = torch.zeros(pyg_data.num_nodes, 1)  # Initialize feature matrix
for node_id, centrality in eigenvector_centralities.items():
    mapped_idx = node_mapping[node_id]  # Map node ID to an integer
    centrality_features[mapped_idx] = normalized_centralities[node_mapping[node_id]]  # Assign normalized centrality

pyg_data.x = centrality_features  # Assign eigenvector centrality as features
print(f"Assigned normalized eigenvector centrality features: {pyg_data.x.shape}")
