...
# Step 1: Compute Clustering Coefficient and Normalize using PyTorch
print("Computing clustering coefficients...")
clustering_coeffs = nx.clustering(nx_graph)  # Compute clustering coefficient for each node
clustering_values = torch.tensor(list(clustering_coeffs.values()), dtype=torch.float)  # Convert to tensor

# Normalize the clustering coefficients to range [0, 1]
min_coeff = torch.min(clustering_values)
max_coeff = torch.max(clustering_values)
normalized_clustering = (clustering_values - min_coeff) / (max_coeff - min_coeff)
...
# Create a feature matrix where each node's feature is its normalized clustering coefficient
clustering_features = torch.zeros(pyg_data.num_nodes, 1)  # Initialize feature matrix
for node_id, coeff in clustering_coeffs.items():
    mapped_idx = node_mapping[node_id]  # Map node ID to an integer
    clustering_features[mapped_idx] = normalized_clustering[node_mapping[node_id]]  # Assign normalized clustering coeff

pyg_data.x = clustering_features  # Assign clustering coefficient as features
print(f"Assigned normalized clustering coefficient features: {pyg_data.x.shape}")
