...
# Step 1: Compute Node Degree, Clustering Coefficient, and Local Betweenness Centrality
print("Computing Node Degree, Clustering Coefficient, and Local Betweenness Centrality...")
...
# Compute Local Betweenness Centrality (2-hop)
local_betweenness_scores = nx.betweenness_centrality(nx_graph, k=2, normalized=True)
local_betweenness_values = torch.tensor(list(local_betweenness_scores.values()), dtype=torch.float)

# Normalize the features
def safe_normalize(values):
    """Normalize values safely to avoid NaNs."""
    min_val = values.min()
    max_val = values.max()
    if max_val - min_val == 0:  # Prevent division by zero
        return torch.zeros_like(values)
    normalized = (values - min_val) / (max_val - min_val)
    normalized[torch.isnan(normalized)] = 0.0  # Replace NaNs with 0
    return normalized

degree_values = safe_normalize(degree_values)
clustering_values = safe_normalize(clustering_values)
local_betweenness_values = safe_normalize(local_betweenness_values)

# Combine the features into a single feature matrix
combined_features = torch.stack([degree_values, clustering_values, local_betweenness_values], dim=1)

# Assign features
pyg_data.x = combined_features
print(f"Assigned combined features (Node Degree + Clustering Coefficient + Local Betweenness Centrality): {pyg_data.x.shape}")
