...
# Step 2: Compute Clustering Coefficient
...
min_clustering = torch.min(clustering_values)
max_clustering = torch.max(clustering_values)
normalized_clustering = (clustering_values - min_clustering) / (max_clustering - min_clustering)
...
print("Combining Node Degree and Clustering Coefficient...")
combined_features = torch.zeros((pyg_data.num_nodes, 2))  # Two features: degree and clustering
for node_id, idx in node_mapping.items():
    combined_features[idx, 0] = normalized_degrees[idx]  # Assign normalized degree
    combined_features[idx, 1] = normalized_clustering[idx]  # Assign normalized clustering
